...
internal PLATFORM_GET_ALL_FILE_BEGIN(Win32GetAllFilesBegin)
{
    ...
    for(u32 extensionIndex = 0; extensionIndex < ext.count; ++extensionIndex)
    {
        ...
        while(findHandle != INVALID_HANDLE_VALUE)
        {
            ...
            info->timestamp.access = ((u64)findData.ftLastAccessTime.dwHighDateTime << (u64) 32) | (u64) findData.ftLastAccessTime.dwLowDateTime;
            
            info->timestamp.write = ((u64)findData.ftLastWriteTime.dwHighDateTime << (u64) 32) | (u64) findData.ftLastWriteTime.dwLowDateTime;
