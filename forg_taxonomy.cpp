...
inline void FreeTaxonomySlot(TaxonomyTable* table, TaxonomySlot* slot)
{    
    FREELIST_FREE(slot->firstEquipmentMapping, EquipmentMapping, table->firstFreeEquipmentMapping);
    FREELIST_FREE(slot->firstConsumeMapping, ConsumeMapping, table->firstFreeConsumeMapping);
    
    for(PlayerPossibleAction* action = slot->firstPossibleAction; action; action = action->next)
    {
        FreePlayerActionTree(table, action->tree.root);
    }
    
    FREELIST_FREE(slot->firstPossibleAction, PlayerPossibleAction, table->firstFreePlayerPossibleAction);
    FREELIST_FREE(slot->firstComponent, TaxonomyComponent, table->firstFreeTaxonomyComponent);
    FREELIST_FREE(slot->essences, TaxonomyEssence, table->firstFreeTaxonomyEssence);
    FREELIST_FREE(slot->links, CraftingEffectLink, table->firstFreeCraftingEffectLink);
    FREELIST_FREE(slot->firstEffect, TaxonomyEffect, table->firstFreeTaxonomyEffect);
    
#ifndef FORG_SERVER
    FREELIST_FREE(slot->plantParams, PlantParams, table->firstFreePlantParams);
    FREELIST_FREE(slot->firstPart, TaxonomyPart, table->firstFreePart);
    FREELIST_FREE(slot->firstVisualTag, VisualTag, table->firstFreeVisualTag);
    FREELIST_FREE(slot->firstAnimationEffect, AnimationEffect, table->firstFreeAnimationEffect);
#endif
    
    FREELIST_FREE(slot->behaviorContent, AIBehavior, table->firstFreeAIBehavior);
    FREELIST_FREE(slot->criteria, MemCriteria, table->firstFreeMemCriteria);
    
	for(MemSynthesisRule* rule = slot->synthRules; rule; rule = rule->next)
    {
        FreeMemSynthRuleTree(table, rule->tree.root);
    }
    
	FREELIST_FREE(slot->synthRules, MemSynthesisRule, table->firstFreeMemSynthesisRule);
    FREELIST_FREE(slot->firstPossibleBehavior, TaxonomyBehavior, table->firstFreeTaxonomyBehavior);
    FREELIST_FREE(slot->firstMemBehavior, TaxonomyMemBehavior, table->firstFreeTaxonomyMemBehavior);
    FREELIST_FREE(slot->nakedHandReq, NakedHandReq, table->firstFreeNakedHandReq);
    
    u32 oldTaxonomy = slot->taxonomy;
    u64 oldStringHashID = slot->stringHashID;
    u32 oldSubTaxonomiesCount = slot->subTaxonomiesCount;
    u32 oldUsedBitstotal = slot->usedBitsTotal;
    
    u32 oldParentNecessaryBits = slot->parentNecessaryBits;
    u32 oldNecessaryBits = slot->necessaryBits;
    
    
    char oldName[32];
    StrCpy(slot->name, StrLen(slot->name), oldName, sizeof(oldName));
    TaxonomySlot* oldNextInHash = slot->nextInHash;
    ...
    {
        oldTabs[tabIndex] = slot->tabs[tabIndex];
    }
    
    *slot = {};
    
    
    slot->taxonomy = oldTaxonomy;
    slot->stringHashID = oldStringHashID;
    slot->subTaxonomiesCount = oldSubTaxonomiesCount;
    slot->usedBitsTotal = oldUsedBitstotal;
    slot->parentNecessaryBits = oldParentNecessaryBits;
    StrCpy(oldName, StrLen(oldName), slot->name, sizeof(slot->name));
    slot->nextInHash = oldNextInHash;
    
    for(u32 tabIndex = 0; tabIndex < oldTabCount; ++tabIndex)
    {
        slot->tabs[tabIndex] = oldTabs[tabIndex];
    }
    slot->tabCount = oldTabCount;
}
