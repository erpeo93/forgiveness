...
internal b32 Edit_u16(EditorLayout* layout, char* name, u16* number, b32 isInArray, AssetID assetID)
{
	...
	if(HotAUIDAndPressed(layout->context, ID, mouseLeft))
    {
		data->speed = 0;
		data->increasing = true;
        Copy(sizeof(u16), data->before, number);
        SetInteractiveAUID(layout->context, ID);
        *number = ++*number;
    }
    ...
    if(IsInteractiveAUID(layout->context, ID))
    {
        r32 real = (r32) *number + data->speed;
		real = Max(real, 0);
		*number = (u16) RoundReal32ToU32(real);
		...
		if(data->increasing)
		{
			...
			if(UIReleased(layout->context, mouseLeft))
			{
                AddUndoRedoCopy(layout->context, sizeof(u16), data->before, number, sizeof(u16), number, assetID);
				EndInteraction(layout->context);
                result = true;
			}
		}
...
internal b32 Edit_GameAssetType(EditorLayout* layout, char* name, GameAssetType* type, b32 typeEditable, b32 isInArray, AssetID assetID)
{
    ...
    if(EditString(layout, "subtype", subtypeString, auID(&type->subtypeHash), subtypeOptions, output, sizeof(output), assetID))
    {
        u64 newHash = StringHash(output);
        if(newHash != 0)
        {
            u64 oldHash = type->subtypeHash;
            type->subtypeHash = newHash;
            AddUndoRedoCopyStruct(u16, layout->context, &oldHash, &type->subtypeHash, &newHash, assetID);
        }
    }
...
internal b32 Edit_EntityName(EditorLayout* layout, char* name, EntityName* ref, b32 isInArray, AssetID assetID)
{
    ...
    char* typeString = GetAssetSubtypeName(assets, AssetType_EntityDefinition, ref->subtypeHashIndex); 
    if(!typeString)
    {
        ref->subtypeHashIndex = 0;
    ...
    if(EditString(layout, "type", typeString, auID(&ref->subtypeHashIndex), kindOptions, output, sizeof(output), assetID))
    {
...
internal void RenderEditAssetFile(GameModeWorld* worldMode, EditorLayout* layout, Assets* assets, PAKFileHeader* header)
{
    ...
    u32 subtype = GetAssetSubtype(assets, type, header->subtype);
