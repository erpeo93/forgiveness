...
{
    PushRect(layout->group, FlatTransform(V4(0, 0, 0, 0.5f)), bounds);
    ...
    for(u32 frameIndex = 0; (frameIndex < frameCount); ++frameIndex)
    {
        ...
        if(highlight)
        {
            PushRect(layout->group, FlatTransform(color), regionRect);
        }
        else
        {
            PushRectOutline(layout->group, FlatTransform(color), regionRect, 2.0f);
        }
...
internal void DrawProfileBars(EditorLayout* layout, DebugCollationState* collation, Rect2 bounds, DebugElement* rootElement, r32 laneStride, r32 laneHeight, u32 depthRemaining)
{
    ...
    if(rootNode)
    {
        ...
        for(DebugProfileNode* node = rootNode->firstChild; node; node = node->nextSameParent)
        {
            ...
            PushRectOutline(layout->group, FlatTransform(V4(0, 0, 0, 1)), regionRect, 2.0f);
            PushRect(layout->group, FlatTransform(V4(color, 1)), regionRect);
            ...
            for(u32 frameIndex = 0; frameIndex < frameCount; ++frameIndex)
            {
                ...
                if(rootNode)
                {
                    ...
                    for(DebugProfileNode* node = rootNode->firstChild; node; node = node->nextSameParent)
                    {
                        ...
                        Rect2 regionRect = RectMinMax(V2(atX, thisMinY), V2(atX + barWidth, thisMaxY));
                        PushRectOutline(layout->group, FlatTransform(V4(0, 0, 0, 1)), regionRect, 2.0f);
                        PushRect(layout->group, FlatTransform(V4(color, highDim)), regionRect);
...
internal void DEBUGOverlay(EditorLayout* layout)
{
    ...
    if(debugState->profiling)
    {
        ...
        if(collation->currentRootElement)
        {
            ...
            PushRect(layout->group, FlatTransform(V4(0, 0, 0, 0.7f)), profilerBounds);
