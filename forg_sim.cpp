internal void EndSim(SimRegion* region)
{
    ...
    for(SimEntityBlock* entityBlock = region->entities; entityBlock; entityBlock = entityBlock->next)
    {
...
JOB_DEFINITION(MoveEntities)
{
    MoveEntitiesParams* params = (MoveEntitiesParams*) params;
    for(everyEntity)
    {
        MoveEntity(params->P[index], params->V[index]);
    }
}

PLATFORM_WORK_CALLBACK(MoveEntities)
{
